...
def imsave1(input_track, target_track, idx, save_dir):
    [batch_size, seqlen, image_size, image_size, num_channel] = np.shape(input_track)
    for i in range(batch_size):
        img = np.zeros((image_size * 2, image_size * seqlen, num_channel))
        for j in range(seqlen):
            img[:image_size, image_size * j: image_size * (j + 1), :] = input_track[i, j]
            img[image_size:, image_size * j: image_size * (j + 1)] = target_track[i, j]
