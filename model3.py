...
def pred(input, hidden, filter = 9):
    # encoder
    h1 = lrelu(batch_norm(conv2d(input, 32, d_h = 1, d_w = 1, name = 'h1'), name='bn1'))
    h2 = lrelu(batch_norm(conv2d(h1, 32, d_h = 2, d_w = 2, name = 'h2'), name='bn2'))
    h3 = lrelu(batch_norm(conv2d(h2, 64, d_h = 1, d_w = 1, name = 'h3'), name='bn3'))
    h4 = lrelu(batch_norm(conv2d(h3, 64, d_h = 1, d_w = 1, name = 'h4'), name='bn4'))
    
    # middle
    h5 = lrelu(batch_norm(conv2d(h4, 128, d_h = 1, d_w = 1, name = 'h5'), name='bn5'))
    hidden1 = lrelu(batch_norm(conv2d(hidden, 128, d_h = 1, d_w = 1, name = 'hidden_1'), name='bn_h_1'))
    hidden2 = lrelu(batch_norm(conv2d(hidden1, 128, d_h = 1, d_w = 1, name = 'hidden_2'), name='bn_h_2'))
    h6 = tf.add(hidden2, h5)
    hidden_state = h6

    #decoder
    h7 = lrelu(batch_norm(conv2d(h6, 64, d_h = 1, d_w = 1, name = 'h7'), name='bn7'))
    h8 = lrelu(batch_norm(conv2d(h7, 64, d_h = 1, d_w = 1, name = 'h8'), name='bn8'))
    h9 = add_upscale(h8)
    h10 = lrelu(batch_norm(conv2d(h9, 64, d_h = 1, d_w = 1, name = 'h10'), name='bn10'))
    h11 = lrelu(batch_norm(conv2d(h10, 64, d_h = 1, d_w = 1, name = 'h11'), name='bn11'))
    h12 = lrelu(batch_norm(conv2d(h11, 128, k_h=1, k_w=1, d_h = 1, d_w = 1, name = 'h12'), name='bn12'))
